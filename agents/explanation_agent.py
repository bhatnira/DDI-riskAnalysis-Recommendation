"""
Explanation Agent - Generates human-readable reports and explanations using LLM (BioMistral-7B)
"""
...
class ExplanationAgent(BaseAgent):
    ...
    def initialize(self, use_llm: bool = True) -> bool:
        """Initialize the explanation agent with optional LLM"""
        ...
        if self.use_llm and self.llm_client is None:
            try:
                ...
                if self.llm_available:
                    ...
                    print(f"⚠️ [{self.name}] LLM not available, using template fallback")
                    print(f"   To enable: ollama serve && ollama pull biomistral")
            except ImportError:
                print(f"⚠️ [{self.name}] LLM client not found, using templates")
                self.llm_available = False
...
"""
    ...
    def _generate_drug_summary(self, validation: Dict) -> str:
        ...
        if valid_drugs:
            ...
            for drug in valid_drugs:
                status = "✓" if drug.get('status') == 'valid' else "~"
    ...
    def _generate_risk_summary(self, risk: Dict) -> str:
        ...
        emoji = self.RISK_EMOJIS.get(risk_level, '❔')
    ...
    def _generate_interaction_details(self, interactions: List[Dict]) -> str:
        ...
        lines = [
            "\n⚠️ DRUG-DRUG INTERACTIONS DETECTED",
            "─" * 50
        ]
    ...
    def _generate_alternatives_section(self, alternatives: Dict) -> str:
        ...
        for drug, best in best_alts.items():
            ...
            lines.append(f"   Safety:  {'★' * int(best.get('safety_score', 0) / 20)}{'☆' * (5 - int(best.get('safety_score', 0) / 20))} ({best.get('safety_score', 0)}/100)")
            
            conflicts = best.get('conflicts_with_current', [])
            if conflicts:
                lines.append(f"   ⚠️ Note: May interact with {', '.join(conflicts[:3])}")
    ...
    def _generate_footer(self) -> str:
        ...
        return """
─────────────────────────────────────────────────────────────────────
⚕️  DISCLAIMER: This AI-generated report is for informational purposes
...
"""
   ...
   ⚠️ IMPORTANT: Do not stop any medication without
