...
We developed the Polypharmacy Risk Index (PRI) to quantify individual drug risk within multi-drug regimens (see Supplementary Section S4 for network analysis methodology). The PRI combines four network-based metrics weighted according to their clinical relevance:
...
The knowledge graph is available as a NetworkX pickle file with complete node and edge attributes, and as Neo4j-compatible CSV files for bulk import. Complete reproducibility information, including software dependencies and execution instructions, is provided in Supplementary Section S5.
