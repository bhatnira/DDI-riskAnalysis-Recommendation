% ============================================================================
% WARNING: THIS DOCUMENT DESCRIBES AN UNIMPLEMENTED APPROACH
% The weighted ensemble formula, semantic similarity, and grid search
% described here were NOT used in the final DDInter validation.
% See methods_brief.tex for the actual validated approach.
% ============================================================================
...
This over-classification phenomenon, while erring on the side of caution, diminishes clinical utility by overwhelming practitioners with false-positive high-severity alerts, contributing to alert fatigue~\citep{van2013frequency}.
...
\end{enumerate}
    \item A \textbf{hybrid recalibration framework} combining multiple evidence sources
    \item \textbf{Clinical text marker analysis} based on pharmacovigilance terminology
    \item \textbf{Drug class risk profiling} for pharmacologically-informed adjustment
    \item \textbf{Comprehensive validation} against clinical benchmarks
\end{enumerate}
...
These thresholds were derived to optimize alignment with DDInter-based target distributions while preserving clinical safety constraints.
...
Initial severity predictions were generated using the \texttt{facebook/bart-large-mnli} model~\citep{lewis2020bart} via zero-shot classification:
...
The recalibration transition matrix reveals systematic downgrading of over-classified interactions:

\begin{table}[H]
\centering
\caption{Severity Transition Matrix}
\label{tab:transition}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Original} & \textbf{$\to$Contra} & \textbf{$\to$Major} & \textbf{$\to$Mod} & \textbf{$\to$Minor} \\
\midrule
Contraindicated & 14,652 & 190,566 & 227,008 & 0 \\
Major & 52 & 12,044 & 291,332 & 23,288 \\
Moderate & 0 & 0 & 24 & 0 \\
Minor & 0 & 0 & 20 & 788 \\
\bottomrule
\end{tabular}
...
The \method{} framework addresses a critical limitation of zero-shot DDI severity classification: the tendency to over-classify interactions as high-severity. This over-classification, while conservative from a safety perspective, contributes to alert fatigue---a well-documented phenomenon where clinicians become desensitized to warnings due to excessive false positives~\citep{ancker2017effects}.

Our recalibration reduces contraindicated classifications by 96.7\% while maintaining 100\% sensitivity for validated high-risk combinations. This selective reduction of false positives could improve the clinical utility of DDI alerting systems.

\subsection{Methodological Considerations}

\subsubsection{Weight Selection}

The weight parameters ($w_s = 0.45$, $w_c = 0.25$, $w_d = 0.30$) were determined via grid search (0.1--0.6, step 0.05) minimizing Jensen-Shannon divergence from DDInter-derived target distributions while maintaining $>$95\% sensitivity for Black Box Warning pairs. The semantic component receives highest weight (0.45) because embedding-based similarity generalizes across diverse DDI descriptions---capturing nuanced clinical phrasing that rule-based approaches miss. Confidence adjustment receives lowest weight (0.25) as a corrective factor applying only to low-confidence predictions. Drug class scoring receives intermediate weight (0.30) to provide pharmacological priors that complement semantic analysis. Sensitivity analysis (Supplementary Figure S1) demonstrates robustness across weight perturbations of $\pm$0.1.

\subsubsection{Marker Taxonomy}

The clinical marker taxonomy represents a synthesis of FDA labeling conventions, pharmacovigilance terminology, and DrugBank description patterns. Future work could extend this taxonomy through systematic extraction from adverse event databases such as FAERS.
