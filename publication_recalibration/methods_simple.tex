...
The weight parameters ($w_s = 0.45$, $w_c = 0.25$, $w_d = 0.30$) were determined via grid search (0.1--0.6, step 0.05) minimizing Jensen-Shannon divergence from DDInter-derived target distributions while maintaining $>$95\% sensitivity for Black Box Warning pairs. The semantic component receives highest weight (0.45) because embedding-based similarity generalizes across diverse DDI descriptions without relying on fixed keyword patterns---capturing nuanced clinical phrasing that rule-based approaches miss. Confidence adjustment receives lowest weight (0.25) as it functions as a corrective factor rather than a primary classification signal, applying only to the subset of predictions with uncertain posterior probabilities. Drug class scoring receives intermediate weight (0.30) to provide pharmacological class-level priors that complement description-level semantic analysis, capturing interactions where generic descriptions may understate risk implied by specific drug categories.
